...
def verify_account():
    ...
    if (regex1.search(username) != None):
    ...
    results = acct.verify_account(username, password)
...
def profile():
    ...
    user_data = acct.fetch_account(username, api_key)
...
def profile_confirmation():
    ...
    # If a match is found, re.search() returns a match object; otherwise, it returns None.
    if (regex1.search(username) != None):
        error_msg = "Special characters are not allowed in username"
        flash(error_msg, "error")
    ...
    if (regex2.search(password) != None):
        error_msg = "Special characters allowed in password are '!', '@', '+', '-', and '_'"
        flash(error_msg, "error")
        return redirect("/create-account")


    results = acct.create_account(username, password)
...
def update_profile():
    ...
    results = acct.update_account(username, password, horizon, risk, api_key)
...
def build_default_portfolio():
    ...
    results = port.create_portfolio(username, portfolio_name, stock_allocation, bond_allocation, cash_allocation, api_key)
...
def save_portfolio():
    ...
    if (regex2.search(portfolio_name) != None):
        error_msg = "Special characters allowed in portfolio name are '!', '@', '+', '-', and '_'"
        flash(error_msg, "error")
    ...
    if portfolio_name == "Default-Portfolio":
        ...
        flash(error_msg, "error")
        if save_type == "edit":
            return redirect(f"/edit-portfolio/{portfolio_name}")
        else:
            return redirect("/custom-portfolio")
    ...
    results = port.create_portfolio(username, portfolio_name, stocks, bonds, cash, api_key)
    ...
    if results["success"] == 1:
        ...
        flash(success_msg, "success")
        return redirect(f"/portfolio-view/{portfolio_name}")
    else:
        error_msg = results["error_msg"]
        flash(error_msg, "error")
        return redirect("/profile")
...
def portfolio_list():
    ...
    portfolios = port.return_portfolios(username, api_key)
    if len(portfolios) < 1:
...
def portfolio_view(portfolio_name):
    ...
    # If a match is found, re.search() returns a match object; otherwise, it returns None.
    if (regex2.search(portfolio_name) != None):
        error_msg = "That URL is invalid. Please log in and try again"
        flash(error_msg, "error")
        return redirect("/")
    ...
    portfolio = port.return_portfolio(username, portfolio_name, api_key)
...
def delete_portfolio(portfolio_name):
    ...
    results = port.delete_portfolio(username, portfolio_name, api_key)
...
def sim_portfolio(portfolio_name):
    ...
    user_stocks = portfolio["stocks"]
    user_bonds = portfolio["bonds"]
    ...
    reinvest_str = request.form.get("reinvest")
    ...
    args["portfolio_name"] = portfolio_name
    args["key"] = api_key
    args["user_stocks"] = user_stocks
    args["user_bonds"] = user_bonds
    args["user_cash"] = user_cash

    args["stock_ret"] = stock_ret
    args["stock_std"] = stock_std
    args["stock_div"] = stock_div

    args["bond_ret"] = bond_ret
    args["bond_std"] = bond_std
    args["bond_div"] = bond_div
